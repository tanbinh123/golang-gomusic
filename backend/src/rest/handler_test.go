...
func TestHandler_GetProducts(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// get 요청
			// GetProducts()를 productsURL에 매핑
			engine.GET(productsURL, h.GetProducts)

			// Gin 엔진이 HTTP 요청을 처리하도록 설정하고 HTTP 응답은 ResponseRecorder 타입으로 생성
			engine.ServeHTTP(w, req)

			// 결과 검증
			response := w.Result()

			// 에러 메시지 출력, 테스트 케이스 실패
			...
			// 에러가 발생한 시나리오와 성공적으로 상품 목록을 반환하는 시나리오 작성
